...
class Depth_Decoder_QueryTr(nn.Module):
    def __init__(self, in_channels, embedding_dim=128, patch_size=16, num_heads=4, query_nums=100, dim_out=256, norm='linear',
        ...
        encoder_layers = nn.modules.transformer.TransformerEncoderLayer(
            embedding_dim, num_heads, dim_feedforward=1024, batch_first=True)
        # encoder_layers = nn.modules.transformer.TransformerEncoderLayer(embedding_dim, num_heads, dim_feedforward=512, batch_first=True) # for resnet18
    ...
    def _get_positional_encodings(self, token_count):
        pe = self.positional_encodings
        if token_count <= pe.shape[0]:
            return pe[:token_count, :]
        repeat = (token_count + pe.shape[0] - 1) // pe.shape[0]
        return pe.repeat(repeat, 1)[:token_count, :]

    # @get_local('x0')
    def forward(self, x0):
        embeddings_0 = self.embedding_convPxP(x0.clone())
        embeddings_0 = embeddings_0.flatten(2)
        pos_enc = self._get_positional_encodings(embeddings_0.shape[2])
        embeddings_0 = embeddings_0 + pos_enc.T.unsqueeze(0)
        embeddings_0 = embeddings_0.transpose(1, 2)
