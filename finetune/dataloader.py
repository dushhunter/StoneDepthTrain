...
def decode_depth_image(depth_pil, depth_encoding, depth_scale):
    """Decode depth PIL image into float32 metres.

    Supported encodings:
      - uint16: depth_m = uint16 / depth_scale
      - float32_rgba: lossless float32 packed in RGBA bytes
      - auto: RGBA -> float32_rgba, otherwise uint16
    """
    encoding = depth_encoding
    if encoding == 'auto':
        encoding = 'float32_rgba' if depth_pil.mode == 'RGBA' else 'uint16'

    if encoding == 'float32_rgba':
        rgba = np.asarray(depth_pil.convert('RGBA'), dtype=np.uint8)
        h, w, c = rgba.shape
        if c != 4:
            raise ValueError(f"Expected RGBA depth image, got shape {rgba.shape}")
        depth = rgba.reshape(-1, 4).view('<f4').reshape(h, w).copy()
        return depth.astype(np.float32)

    # Legacy integer depth path.
...
class DataLoadPreprocess(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            image = image.resize((self.args.input_width, self.args.input_height), Image.Resampling.BILINEAR) # force image size
            depth_gt = depth_gt.resize((self.args.input_width, self.args.input_height), Image.Resampling.NEAREST) # force depth gt size
            
            depth_encoding = getattr(self.args, 'depth_encoding', 'uint16')

            if self.args.do_random_rotate is True and self.args.degree > 0:
                if depth_encoding == 'float32_rgba':
                    raise ValueError('Random rotation is not supported for float32 RGBA depth encoding due to potential interpolation issues. Please set do_random_rotate to False or use uint16 encoding.')
            ###<<<CURSOR    
            ...
            #depth_encoding = getattr(self.args, 'depth_encoding', 'uint16')
            ###<<<CURSOR
            depth_gt = decode_depth_image(depth_gt, depth_encoding, self.args.depth_scale)
